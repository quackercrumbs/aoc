...
(defn process-monkey
  [{items :items operation :operation test :test true-id :true false-id :false} stress-mod-fn]
  (reduce (fn [to-return item]
            (let [{:keys [op arg]} operation
                  ...
                  modified-worry (stress-mod-fn modified-worry)]
...
(deftest test-process-monkey
  ...
  (let [result (process-monkey {:items [10 20 30]
                                ...
                                :test 2
                                :true 1
                                :false 2} (partial mod-fn 2))]
    (is (= {2 [33 133] 1 [300]} result))))

(defn process-monkeys
  [monkeys]
  ;; We can use the LCM since the input test modulos functions are all prime!
  (let [lcm (->> (vals monkeys) (mapv :test) (apply *))]
    (reduce (fn [monkeys monkey-id]
              (let [monkey (get monkeys monkey-id)
                    num-inspected (count (get-in monkeys [monkey-id :items]))
                    processed-monkey (process-monkey monkey (partial mod-fn lcm))
...
(deftest test-sample-p2
  (is (= 10197 (->> (calculate-monkey-business (get-sample-input) 20)
                    :monkey-business))))
...
(defn problem-2
  ...
  (let [lines (with-open [file (io/reader "/home/calvinq/projects/aoc/2022/day11/input.txt")]
                (doall (line-seq file)))]
    (calculate-monkey-business lines 10000)))
(time (->> (problem-2)
           :monkey-business))
; 99852
; 2.6ms
